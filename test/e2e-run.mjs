...
const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "hwh5-"));
let failures = 0;
...
const finished = await until(()=>sb.evaluate(()=>document.getElementById("hdr").textContent==="4 of 4 Concepts completed"));
check("ran through all 4 questions unattended", finished,
  await sb.evaluate(()=>document.getElementById("hdr").textContent));
check("answers recorded in order", 
  JSON.stringify(await sb.evaluate(()=>window.__answers))===JSON.stringify(["beta","one","down","left"]),
  JSON.stringify(await sb.evaluate(()=>window.__answers)));
check("loop stopped by itself at the end",
  await until(()=>sb.locator("#hw-helper-trigger").textContent().then(t=>t==="HW Helper"),30000),
  await btn.textContent());
